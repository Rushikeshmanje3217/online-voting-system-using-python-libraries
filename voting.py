...
from tkinter import messagebox #poll simple templete boxes
from tkinter import simpledialog # contains convenience classes and functions making body and button box i.e syntax
from tkinter import filedialog as fd #this function use for single file or button selection
import sqlite3 as sqltor #create a connection to a SQLite database
import matplotlib.pyplot as plt #used for make diagram like pie
# # Libraries Used
# 1. Tkinter
# 2. Matplotlib
# 3. Sqlite3
...
conn=sqltor.connect('main.db') #main database stored for each poll
...
def polls(): #mypolls
    ...
    mpolls.title('Voting system Program')
...
def about():
    messagebox.showinfo('About','Developed by Rushikesh')


home=Tk()
home.geometry('400x400')
home.title('Voting System Program')
home['bg'] = '#FFd700'
Label(home,text='voting program made in python',font='Helvetica 19 bold',bg='#FFd700').grid(row=1,column=2)
Button(home,text='Create new Poll +',command=create ,font='Helvetica 19 bold' ,bg='#FF0000').grid(row=3,column=2)
Button(home,text='My Polls',command=polls ,font='Helvetica 19 bold' ,bg='#FF0000').grid(row=4,column=2)
Button(home,text='Poll Results',command=selpl ,font='Helvetica 19 bold' ,bg='#FF0000').grid(row=5,column=2)
Label(home,text='Rushikesh manje Project',bg='#FFd700').grid(row=6,column=2)
Label(home,text='Skill based lab Course',bg='#FFd700').grid(row=7,column=2)
Button(home,text='About',command=about).grid(row=2,column=2)
